...
<div class="container mx-auto px-4 py-6">
    ...
    <div class="flex items-center mb-6">
        ...
        <div>
            <h1 class="text-3xl font-bold text-gray-900 dark:text-white">Editar Mascota</h1>
            <p class="text-gray-600 dark:text-gray-400 mt-1">Modifica la información de {{ $pet->name }}</p>
        </div>
    </div>

    <!-- Formulario -->
    <div class="bg-white dark:bg-gray-800 rounded-lg shadow-md border border-gray-200 dark:border-gray-700">
        <form action="{{ route('admin.pets.update', $pet) }}" method="POST" class="p-6 space-y-6">
            @csrf
            @method('PUT')
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <select name="species" id="species" required
                            ...
                            <option value="dog" {{ old('species', $pet->species) === 'dog' ? 'selected' : '' }}>Perro</option>
                            <option value="cat" {{ old('species', $pet->species) === 'cat' ? 'selected' : '' }}>Gato</option>
                            <option value="other" {{ old('species', $pet->species) === 'other' ? 'selected' : '' }}>Otro</option>
                        </select>
                        @error('species')
                            <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                        @enderror
                    ...
                    <div>
                        ...
                        <input type="number" name="weight" id="weight" value="{{ old('weight', $pet->weight) }}" min="0" max="200" step="0.1"
                               class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 bg-white dark:bg-gray-700 text-gray-900 dark:text-white">
                        @error('weight')
                            <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                        @enderror
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <select name="caretaker_id" id="caretaker_id"
                            ...
                            @foreach($caretakers as $caretaker)
                                <option value="{{ $caretaker->id }}" 
                                        ...
                                        {{ old('caretaker_id', $pet->caretaker_id) == $caretaker->id ? 'selected' : '' }}>
                    ...
                    <div>
                        ...
                        <select name="adoption_status" id="adoption_status" required
                            ...
                            <option value="available" {{ old('adoption_status', $pet->adoption_status) === 'available' ? 'selected' : '' }}>Disponible</option>
                            <option value="pending" {{ old('adoption_status', $pet->adoption_status) === 'pending' ? 'selected' : '' }}>En proceso</option>
                            <option value="adopted" {{ old('adoption_status', $pet->adoption_status) === 'adopted' ? 'selected' : '' }}>Adoptado</option>
                            <option value="inactive" {{ old('adoption_status', $pet->adoption_status) === 'inactive' ? 'selected' : '' }}>Inactivo</option>
                        </select>
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                    ...
                    <div class="flex items-center">
                        <input type="checkbox" name="is_sterilized" id="is_sterilized" value="1" 
                               {{ old('is_sterilized', $pet->is_sterilized) ? 'checked' : '' }}
                ...
                <div class="space-y-6">
                    ...
                    <div>
                        ...
                        <textarea name="description" id="description" rows="3"
                                  class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 bg-white dark:bg-gray-700 text-gray-900 dark:text-white"
                                  placeholder="Describe las características y personalidad de la mascota...">{{ old('description', $pet->description) }}</textarea>
                        @error('description')
                            <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <textarea name="medical_info" id="medical_info" rows="3"
                                  class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 bg-white dark:bg-gray-700 text-gray-900 dark:text-white"
                                  placeholder="Información sobre tratamientos, condiciones médicas, etc...">{{ old('medical_info', $pet->medical_info) }}</textarea>
                        @error('medical_info')
                            <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <textarea name="behavior_notes" id="behavior_notes" rows="3"
                                  class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 bg-white dark:bg-gray-700 text-gray-900 dark:text-white"
                                  placeholder="Comportamiento con otros animales, personas, niños, etc...">{{ old('behavior_notes', $pet->behavior_notes) }}</textarea>
            ...
            <div class="flex justify-end space-x-4 pt-6 border-t border-gray-200 dark:border-gray-700">
                ...
                <button type="submit" 
                        class="px-6 py-2 bg-gradient-to-r from-blue-600 to-blue-700 text-white rounded-lg hover:from-blue-700 hover:to-blue-800 transition duration-200 shadow-lg">
